...
<div class="player">
    <!-- <div class="info">
        ...
        <nuxt-link :to="'/track/' + song.title" class="title">{{ song.title }}</nuxt-link>
    </div> -->
    <div class="actions">
...
<path fill="currentColor" d="M16,4.995v9.808C16,15.464,15.464,16,14.804,16H4.997C4.446,16,4,15.554,4,15.003V5.196C4,4.536,4.536,4,5.196,4h9.808C15.554,4,16,4.446,16,4.995z"/>		</svg>		</a>
        ...
        <div class="progress-container">
            <div v-on:click="seek" ref="seekbar" class="player-progress" title="Time played : Total time" v-bind:class="{ loading: this.loading }">
                ...
                <div :style="{ width: likePosition(like.song_timestamp) + '%' }" class="player-time-like" v-for="like in song.likes" :key="like.id">&nbsp;</div>
            </div>
...
export default {
    ...
    computed: {
        likeCount() {
            ...
            let count = 0;
            for (var like in this.song.likes) {
                count = count + 1;
            }
            return count;
        },
        ...
        liked() {
            const values = Object.values(this.song.likes);
        ...
        albumart() {
            if (this.song.albumart) return this.song.albumart.data.full_url
            else return ""
        },
        loading() {
            // console.log()
    ...
    watch: {
        playing(value) {
            if (this.mounted) {
                if (value) {
                    ...
                    if (this.skim) this.$store.commit('skim', this.song.hotspot)
                    return;
                }
                console.log("pause");
                this.$store.state.playing = value;
        ...
        playingSong(value) {
            if (value.song.id == this.song.id) this.playing = true;
    ...
    mounted() {
        this.durationSeconds = this.timeStringToFloat(this.song.length);
        // if (
        //     this.$store.state.song &&
        //     this.$store.state.song.id == this.song.id &&
        //     this.$store.state.playing
        // ){this.playing = true;
        // }
        // else {
        //     console.log('not this song playing')
        // }
        // else this.playing = false
        if (this.playing) console.log(this.song.title + " is playing");
